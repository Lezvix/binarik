    ...
    flushBits() {
        ...
        const readBytes = (numBits: number): number => {
            ...
            if (numBits <= 8) {
                ...
                this.push(
                    `var ${val} = (dv.getUint16(off)<<8)|dv.getUint8(off+2); off+=3;`,
                );
                return 24;
            } else {
        ...
        switch (field.kind) {
            ...
            case "int": {
                ...
                if (bits === 8) {
                    ...
                    ctx.push(
                        `${tgt}.${name} = dv.get${signed ? "BigInt64" : "BigUint64"}(off,${le}); off+=8;`,
                    );
                } else {
                    ctx.push(
                        `${tgt}.${name} = dv.get${T}${bits}(off,${le}); off+=${bits / 8};`,
                    );
                }
                break;
            }
            case "float": {
                ...
                const le = field.le !== undefined ? field.le : ctx.le;
                ctx.push(
                    `${tgt}.${name} = dv.get${bits === 32 ? "Float32" : "Float64"}(off,${le}); off+=${bits / 8};`,
                );
                break;
            }
            case "bit": {
            ...
            case "buffer": {
                ...
                const lenExpr =
                    typeof field.length === "number"
                        ...
                        : `${tgt}.${field.length}`;
                ctx.push(
                    `${tgt}.${field.name} = buf.subarray(off, off+${lenExpr}); off+=${lenExpr};`,
                );
                break;
            }
            case "array": {
                ...
                if (typeof itemType === "string") {
                    ...
                    ctx.push(
                        `${tgt}.${name}.push(dv.${info.get}(off${leArg})); off+=${info.size};`,
                    );
                } else {
                    const item = ctx.tmp();
                    ctx.push(`var ${item} = {};`);
    ...
    flushBits() {
        ...
        const writeWord = () => {
            if (sum === 8) {
                ...
                this.push(
                    `dv.setUint16(off,(${val}>>8)&0xffff); dv.setUint8(off+2,${val}&0xff); off+=3;`,
                );
            } else {
        ...
        for (let i = 0; i < pending.length; i++) {
            ...
            if (length > rem) {
                if (rem > 0) {
                    ...
                    this.push(
                        `${val} |= (((${fieldExpr} >>> ${length - rem}) & 0x${(mask >>> 0).toString(16)}) << ${shift});`,
                    );
                    writeWord();
                    length -= rem;
            ...
            this.push(
                `${val} |= ((${fieldExpr} & 0x${(mask >>> 0).toString(16)}) << ${offset});`,
            );
        ...
        switch (field.kind) {
            ...
            case "int": {
                ...
                if (bits === 8) {
                    ...
                    ctx.push(
                        `dv.set${signed ? "BigInt64" : "BigUint64"}(off,BigInt(${src}.${name}),${le}); off+=8;`,
                    );
                } else {
                    ctx.push(
                        `dv.set${T}${bits}(off,${src}.${name},${le}); off+=${bits / 8};`,
                    );
                }
                break;
            }
            case "float": {
                ...
                const le = field.le !== undefined ? field.le : ctx.le;
                ctx.push(
                    `dv.set${bits === 32 ? "Float32" : "Float64"}(off,${src}.${name},${le}); off+=${bits / 8};`,
                );
                break;
            }
            case "bit": {
            ...
            case "array": {
                ...
                if (typeof itemType === "string") {
                    ...
                    const val = info.bigint
                        ...
                        : `${src}.${name}[${i}]`;
                    ctx.push(
                        `dv.${info.set}(off,${val}${leArg}); off+=${info.size};`,
                    );
                } else {
                    genEncodeFields(
                        itemType.fields,
            ...
            case "choice": {
                ...
                ctx.push(
                    `default: throw new Error("Unknown choice tag: "+${src}.${field.tag});`,
                );
                ctx.push(`}`);
                break;
            }
...
function compileEncoder(fields: FieldDescriptor[]): (obj: any) => Uint8Array {
    ...
    ctx.flushBits();
    ctx.push(
        fixedSize !== null ? `return buf;` : `return buf.subarray(0,off);`,
    );
...
// Debug helper
// =============================================================================
export function getGeneratedCode(fields: FieldDescriptor[]): {
    decode: string;
    encode: string;
} {
    ...
    eCtx.push(
        fixedSize !== null
            ? `var buf = new Uint8Array(${fixedSize});`
