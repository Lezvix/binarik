...
export interface KaitaiStruct {
    ...
    types?: Record<
        string,
        { seq: KaitaiSeqEntry[]; types?: Record<string, any> }
    >;
}
...
export function toKaitaiStruct(
        ...
        for (const field of descs) {
            switch (field.kind) {
                ...
                case "int": {
                    ...
                    seq.push({
                        id: field.name,
                        type: intTypeToKaitai(field.bits, field.signed, le),
                    });
                    break;
                }
                ...
                case "float": {
                    ...
                    seq.push({
                        id: field.name,
                        type: floatTypeToKaitai(field.bits, le),
                    });
                    break;
                }
                ...
                case "array": {
                    ...
                    if (typeof length === "function") {
                        throw new Error(
                            "Function-based array length cannot be exported to Kaitai Struct",
                        );
                    }
                ...
                case "nested": {
                    if (field.name) {
                        const typeName = registerSubtype(
                            field.name,
                            field.parser.fields,
                            defaultLe,
                        );
                        ...
                        seq.push(
                            ...convertFields(field.parser.fields, defaultLe),
                        );
                    }
                    break;
                }
                ...
                case "choice": {
                    ...
                    for (const [k, sub] of Object.entries(choices)) {
                        ...
                        cases[Number(k)] = registerSubtype(
                            base,
                            sub.fields,
                            defaultLe,
                        );
